...
const Dashboard = () => {
  ...
  const { fetchUserProfile, getCookie, getUserDevices } =
    ...
    const fetchData = async () => {
      try {
        ...
        if (
          userDevices &&
          userDevices.data &&
          Array.isArray(userDevices.data)
        ) {
          if (userDevices.data.length === 0) {
            setDevices(
              "There are no devices now, click the button below to create one"
            );
          } else {
    ...
    <>
      ...
      <div className=" w-full px-4 md:px-20 py-8 rounded-[20px]">
        <div className=" w-full p-6 bg-[#ECF6E7] flex flex-col lg:flex-row md:items-center rounded-[20px]">
          ...
          <p className=" text-left w-full lg:ml-6 font-Afacad font-semibold text-3xl lg:text-[32px]">
            Welcome Back, {name}
          </p>

          <div className=" flex w-full space-x-6 mt-6 md:mt-8 lg:mt-0 md:space-x-12 lg:ml-auto">
            <span className="">
              <p className=" text-[#0000004D] font-medium font-Afacad text-sm">
                Numbers of Transactions
              </p>
              <p className=" font-Afacad font-medium text-xl lg:text-xl text-[#0000004D] mt-2">
                {transactionCount}
              </p>
            </span>
            <span className="">
        ...
        <div className=" w-full p-6 bg-[#ECF6E7] rounded-[20px] -z-10 mt-8">
          ...
          <div className=" mt-6 flex flex-col lg:flex-row justify-between w-full -z-10 space-y-8 lg:space-y-0 ">
            <div className=" w-full lg:w-[32%] p-6 rounded-[20px] bg-white relative">
              <p className=" font-Afacad text-xl mb-3 font-semibold">
                Authorized Devices
              </p>
                ...
                {devices === null ? (
                  ...
                  <div>
                    {devices.map((device, index) => (
                      <div
                        key={index}
                        className="w-full h-[45px] rounded-[5px] border border-[#EAEBF080] mb-3 flex flex-row justify-between items-center p-[18px]"
                      >
                        <p className="text-base font-Afacad text-[#000000B2] font-normal">
                          {device.name}
                        </p>
              ...
              {deleteMessage && (
                <p className=" font-medium text-sm font-Afacad text-[#636363]">
                  {deleteMessage}
                </p>
              )}
            </div>
            <div className=" w-full relative lg:w-[32%] p-6 rounded-[20px] bg-white">
              <div className=" flex flex-row items-center justify-between mb-6">
                ...
                <button className=" flex flex-row bg-[#78C257] px-3 h-[36px] space-x-2 rounded-[50px] items-center">
                  <img src={crown} alt="" />
                  <p className=" font-medium font-Afacad text-lg text-white">
                    {plan}
                  </p>
                </button>
              </div>
              <div className=" h-[300px] lg:h-auto">
                {plans.map((item, index) => (
                  ...
                  >
                    <span>
                      <p className="font-Afacad text-xs text-[#344054] font-medium">
                        {item.name}
                      </p>
                      <p className="font-Afacad font-normal text-xs text-[#667085]">
                    ...
                    <div className="round">
                      <input
                        ...
                        id={item.name}
                        onChange={(event) =>
                          handlePlanSelection(event, item.name)
                        }
              ...
              <button
                ...
                className=" absolute bottom-6 right-6 mt-[20px] flex ml-auto px-5 py-3 flex-row items-center justify-center bg-[#78C257] rounded-[50px]"
              >
                {loading === "NO" && (
            ...
            <div className=" w-full lg:w-[32%] p-6 rounded-[20px] bg-white">
              <p className=" font-Afacad text-xl mb-[34px] font-semibold">
                Support
              </p>
              <label
                htmlFor=""
                className=" w-full font-Afacad font-medium text-sm text-[#272D37]"
              >
                Reason For Contact
                <select
                  name="reason"
                  ...
                  className=" w-full h-[46px] font-Afacad mt-2 border border-[#DAE0E6] p-2.5 rounded-md"
                >
              ...
              <div className=" mt-6">
                <label
                  htmlFor=""
                  className=" w-full font-Afacad font-medium text-sm"
                >
                  Message
                  <textarea
                    name="message"
                    ...
                    onChange={handleChange}
                    placeholder="Hi! We are an AI transcription company..."
                    className="w-full font-Afacad mt-2 border border-[#DAE0E6] p-2.5 rounded-md"
                    id=""
                    cols="30"
                    rows="3"
                  ></textarea>
                ...
                {responseMessage && (
                  <p className=" mt-2 font-medium font-Afacad text-sm text-[#626262]">
                    {responseMessage}
                  </p>
                )}
                <button
