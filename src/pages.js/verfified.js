...
const Verified = () => {
    ...
    const verifyEmail = async (email, accessCode) => {
        try {
            const response = await fetch('https://dev-api.zoemed.ai/api/v1/auth/email-verify', {
                method: 'POST',
                headers: {
            ...
            <div className="w-full md:w-[500px] h-[150px] bg-[#ECF6E7] shadow p-6">
                {verificationStatus === 'loading' && (
                    <div className="flex items-center justify-center">
                        ...
                        <p className="font-Afacad text-base mt-1 font-normal text-[#4d4d4d] ml-2">Verifying email...</p>
                    </div>
                )}
                {verificationStatus === 'success' && (
                    <>
                        ...
                        <p className="text-[#4d4d4d] text-base mt-1 font-normal font-Afacad">Your Email has been verified successfully</p>
                ...
                {verificationStatus === 'failed' && (
                    <>
                        ...
                        <p className="text-[#4d4d4d] text-base mt-1 font-normal font-Afacad">Email Verification Failed, Please try again.</p>
                        <Link to="/dashboard" className="w-full">
                            <button className="bg-[#78C257] block ml-auto mt-4 px-4 py-2 rounded-[10px] text-white text-sm font-medium font-Afacad">
                                Return to Dashboard
                            </button>
