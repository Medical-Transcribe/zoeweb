  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}api/v1/set-payment-methods`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <>
      <form className="mt-6" onSubmit={handleSubmit}>
        <CardElement />
        <button
          className="w-full h-[45px] rounded-[36px] flex items-center justify-center bg-[#78C257] mt-6 font-Afacad text-center text-white text-base font-medium"
          type="submit"
        ...
        {errorMessage && (
          <div className="font-Afacad text-sm font-medium w-full text-left mt-3">
            {errorMessage}
          </div>
        )}
      </form>
